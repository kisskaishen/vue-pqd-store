 ...
 <div class="marketing">
        ...
        <div class="detail_warp">
            <div class="detail_warp_active">
                <div class="warp">
                    ...
                    <div class="sign_btn" v-show="this.is_type!='unabled'"><el-button size="small" @click="immediately()" type="danger">{{tableDataActive.length==0?'立即报名':'继续报名'}}</el-button></div>
                </div>
            </div>
  ...
  <el-table
    ...
    <el-table-column
      label="活动价格(元)">
        <template slot-scope="scope">
            <el-input size="small" :maxlength="8" v-model.trim="scope.row.limit_price" placeholder="设置活动价格"></el-input>
        </template>
    </el-table-column>
    <el-table-column
      prop="store_count"
 ...
 export default {
   ...
   created(){
       ...
       this.is_type = this.$route.query.type;
        ...
        Market.storeActiveInfo("Activities/activeInfo",data,this)
   },
   components: {
        countDown
   },
   methods:{
       btnActive(){
			var val = this.tableDataObj[0].limit_price;
			if(isNaN(val)){
				this.$message({
					type:'info',
					message:'请输入数字'
       		...
       		var tableDataObj = JSON.stringify(this.tableDataObj);
           var data = {
              activity_id:this.activity_id,
              action:this.action,
              goods_id:this.goods_id,
              specList:tableDataObj,
              id:this.id
           }
            Market.updateActiveGoodsSpec("Activities/updateActiveGoodsSpec",data,this)
       },
       immediately(){
        this.$router.push({path:"/home/marketing/resource_activity_goods",query:{activity_id:this.activity_id,action:this.action}})
       },
       callback(val){
       }, 
       handleClickDetail(tab, event) {
	        if(tab.name=="third"){
	        	this.storeActiveGoodsListFetch()
	        }
       },
       storeActiveGoodsListFetch(){
            var data = {
                activity_id : this.activity_id,
                action : this.action,
             ...
             Market.storeActiveGoodsList("Activities/activeGoodsList",data,this)
       },
      ...
      handleClickDel(params,index){
        ...
        Market.storeDelGoods("Activities/delGoods",data,this,index)
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消删除'
